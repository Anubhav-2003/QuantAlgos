...
The production loader now lives in \texttt{src/load\_daily\_prices.py}. It reads tickers from \texttt{tickers\_metadata}, downloads daily OHLCV data plus \texttt{adjusted\_close} with \texttt{yfinance}, writes into \texttt{daily\_prices}, and can resume from the latest date already stored.

\texttt{yfinance}'s \texttt{download()} function fetches price history for a list of tickers in batched requests. For 2,645 tickers fetching from 2010--01--01 to present, this takes roughly \textbf{30--90 minutes} depending on network speed. Run it once and then use resume mode for later refreshes.
...
\section{Phase 3: Annual Fundamentals via ROIC.ai}

ROIC.ai is now the sole fundamentals source for v1. The AAPL canary check showed a stable annual financials page at \texttt{/quote/AAPL/financials} plus a fallback classic page at \texttt{/quote/AAPL/classic}. The annual tables parse cleanly with \texttt{pandas.read_html}, and for AAPL the visible annual coverage runs from 2011 through 2025. That is enough to seed the loader and validate the pipeline end to end.

The implementation lives in \texttt{src/load_roicai_fundamentals.py}. It preloads resume state once, skips tickers that are already complete through the last closed annual year, fetches the remaining tickers in concurrent batches, and commits each batch in a single transaction. That gives us a rerunnable loader that finishes in minutes instead of a serial, one-ticker-at-a-time crawl.

\begin{lstlisting}[style=pythonstyle, caption={Annual Fundamentals via ROIC.ai (Batched and Concurrent)}]
def load_fundamentals(..., batch_size: int = 80, workers: int = 12):
    resume_states = load_ticker_states()
    selected_tickers = prefilter_completed_tickers(selected_tickers, resume_states)
    for batch in chunked(selected_tickers, batch_size):
        with ThreadPoolExecutor(max_workers=workers):
            fetch and parse each ticker in parallel
        upsert all parsed rows for the batch in one transaction
\end{lstlisting}

\begin{warningbox}
\textbf{ROIC.ai Annual Coverage.} In the AAPL canary test, ROIC.ai exposed annual history from 2011--2025. Exact start year can vary by ticker, so the loader should retain nulls for missing years instead of fabricating values. In practice, many tickers are already complete through 2025, so the loader now skips those before any network work and only revisits the incomplete or unavailable names. The annual-only build is enough to validate the fundamentals pipeline now; if you later want quarterly or strict filing-date correctness, that becomes a separate extension.
\end{warningbox}
